...
#include <algorithm>
#include "MontBlanc/NNADparameterisation_old.h"

namespace MontBlanc
{
  //_________________________________________________________________________
  NNADparameterisation_old::NNADparameterisation_old(YAML::Node const &config, std::shared_ptr<const apfel::Grid> g):
  ...
  //_________________________________________________________________________
  void NNADparameterisation_old::EvaluateOnGrid()
  {
  ...
  //_________________________________________________________________________
  void NNADparameterisation_old::DeriveOnGrid()
  {
  ...
  //_________________________________________________________________________
  std::function<apfel::Set<apfel::Distribution>(double const&)> NNADparameterisation_old::DistributionFunction() const
  {
    return [=] (double const &) -> apfel::Set<apfel::Distribution> { return _NNderivativeSets[0]; };
  }

  //_________________________________________________________________________
  std::function<apfel::Set<apfel::Distribution>(double const&)> NNADparameterisation_old::DistributionDerivative(int ipar) const
  {

...
FIT_LOG_FOLDER=logs/logs_${runcard_name%".yaml"}
FIT_LOG_FOLDER=$FIT_LOG_FOLDER/fit
mkdir -p $FIT_LOG_FOLDER

$qsubpath -N "MontBlanc_"${runcard_name%".yaml"} -e $FIT_LOG_FOLDER -o $FIT_LOG_FOLDER -t 1-$nrep -l h_vmem=2500M -pe sharedmem 4 optimize.sh $runcard $fit_folder
